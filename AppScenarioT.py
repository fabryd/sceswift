...
# ---- Helpers: policy High-Risk ----
def collect_all_countries(inbound_df: Optional[pd.DataFrame], outbound_df: Optional[pd.DataFrame]) -> list[str]:
    cols = [
        "Creditor FI Country Name",
        "Debtor FI Country Name",
    ...
    for df in (inbound_df, outbound_df):
        ...
        for c in cols:
            if c in df.columns:
                s.update(x for x in df[c].dropna().astype(str).unique())
    return sorted(s)
...
def parse_policy_upload(file) -> dict:
    ...
    if "highrisk" in cols_lc:
        ...
        df[highrisk_col] = (
            df[highrisk_col].astype(str).str.strip().str.lower()
    ...
    for _, row in df.iterrows():
        ...
        if not ctry:
            continue
        mapping[ctry] = bool(row.get(highrisk_col, False))
    return mapping
    ...
    for df in (inbound_df, outbound_df):
        ...
        for country_col, risk_col in pairs:
            if country_col in df.columns and risk_col in df.columns:
                ...
                if tmp.empty:
                    continue
                mask = tmp[risk_col].astype(str).str.strip().str.lower().str.contains("high")
                if mask.any():
                    for c in tmp.loc[mask, country_col].astype(str).str.strip():
                        if c:
                            res[c] = True
    return res
...
# ---------------------- Filtri dinamici post-import ----------------------
def _unique_values(sample_series, max_unique=5000):
    try:
        vals = sample_series.dropna().unique()
...
def apply_dynamic_filters(df: pd.DataFrame, scope_label: str):
    """Renderizza in sidebar i controlli di filtro dinamici per df e ritorna il df filtrato.
    - Per colonne object/categoria: multiselect sui valori + filtro 'contiene' opzionale
    - Per colonne numeriche: slider range min/max
    - Per colonne datetime-like: date_input range
    """
    if df is None or df.empty:
        return df

    with st.sidebar:
        st.markdown(f"### 🔎 Filtri {scope_label}")
        # --- Filtri "pinnati" richiesti ---
        pinned_cols = [("My Role Description","my_role_desc"), ("My BIC8 Name","my_bic8")]
        for pname, pkey in pinned_cols:
            if pname in df.columns:
                st.markdown(f"**{pname}**")
                # Multiselect dei valori distinti (se cardinalità ragionevole) + contains
                s = df[pname]
                uniques = _unique_values(s, max_unique=5000)
                pick = st.multiselect(f"Valori {pname}", uniques or [], key=f"{scope_label}_{pkey}_vals")
                if pick:
                    df = df[df[pname].isin(pick)]
                contains_val = st.text_input(f"Contiene in {pname}", key=f"{scope_label}_{pkey}_contains", placeholder="testo…")
                if contains_val:
                    df = df[df[pname].astype(str).str.contains(contains_val, case=False, na=False)]
        # ----------------------------------
    
        cols = list(df.columns)
        sel_cols = st.multiselect(f"Scegli le colonne da filtrare ({scope_label})", cols, default=[],
                                  key=f"filters_pick_{scope_label}")
        filtered = df.copy()
        for c in sel_cols:
            s = filtered[c]
            # Datetime
            if pd.api.types.is_datetime64_any_dtype(s):
                min_d, max_d = pd.to_datetime(s.min()), pd.to_datetime(s.max())
                val = st.date_input(f"{c} – intervallo", value=(min_d.date(), max_d.date()), key=f"{scope_label}_{c}_date" )
                if isinstance(val, tuple) and len(val)==2:
                    start, end = pd.to_datetime(val[0]), pd.to_datetime(val[1]) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
            ...
            # Numeric
                ...
                rng = st.slider(f"{c} – range", min_value=min_v, max_value=max_v, value=(min_v, max_v), key=f"{scope_label}_{c}_num")
                filtered = filtered[(filtered[c] >= rng[0]) & (filtered[c] <= rng[1])]
            else:
                uniques = _unique_values(s)
                if uniques is not None and 1 < len(uniques) <= 2000:
                    pick = st.multiselect(f"{c} – valori", uniques, key=f"{scope_label}_{c}_cat")
                    if pick:
                        filtered = filtered[filtered[c].isin(pick)]
                # free text contains
                txt = st.text_input(f"{c} – contiene", key=f"{scope_label}_{c}_contains", placeholder="testo…")
                if txt:
                    filtered = filtered[filtered[c].astype(str).str.contains(txt, case=False, na=False)]
        st.caption("I filtri si applicano istantaneamente a tabelle, grafici ed export.")
    return filtered
...
def _normalize_share_link(url: str) -> str:
    ...
    if "drive.google.com" in u and "/file/d/" in u and "/view" in u:
        try:
            file_id = u.split("/file/d/")[1].split("/")[0]
            u = f"https://drive.google.com/uc?export=download&id={file_id}"
        except Exception:
            pass
    # OneDrive: shorten to direct download if present
    if "1drv.ms" in u and "redir" not in u and "download" not in u:
        # Many OneDrive shared links accept ?download=1
        sep = "&" if "?" in u else "?"
        u = f"{u}{sep}download=1"
    return u

def read_excel_from_url(url: str, header_index: int = HEADER_ROW_INDEX) -> pd.DataFrame | None:
    try:
        if not url:
            return None
        norm = _normalize_share_link(url)
        r = requests.get(norm, timeout=60)
        r.raise_for_status()
        bio = io.BytesIO(r.content)
        df = pd.read_excel(bio, header=header_index, engine="openpyxl")
        df = df.dropna(how="all")
        df.columns = df.columns.map(lambda c: str(c).strip()).map(lambda c: re.sub(r"\s+", " ", c))
        return df
    except Exception as e:
        try:
            st.warning(f"Impossibile leggere l'URL: {url}\n{e}")
        except Exception:
            pass
        return None


# ---------------------- Periodi & Aggregazioni ----------------------
DATE_COL_PATTERN = re.compile(r"(?i)input\s*date")  # 'Input Date (dd Mon yyyy)'
MEASURE_COL_PREFERRED = "Amount received (converted in EUR)"
AMOUNT_IN_COLS = [
    "Amount received (converted in EUR)",
    "Amount sent (converted in EUR)",
    "Amount received",
    "Amount sent",
]
MSG_IN_COLS = [
...
def _find_col(cols, patterns):
    # prefer exact preferred measure if present
    if isinstance(cols, (list, tuple)) and "Amount received (converted in EUR)" in cols:
        return "Amount received (converted in EUR)"
    for p in patterns:
        for c in cols:
            if (isinstance(p, str) and c.strip()==p) or (hasattr(p, "search") and p.search(c)):
                return c
    return None
...
def ensure_datetime(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty: 
        return df
    date_col = _find_col(df.columns, [DATE_COL_PATTERN, "Input Date (dd Mon yyyy)", "Input Date"])
    if date_col is None:
        return df
    df = df.copy()
    try:
...
def add_period_column(df: pd.DataFrame, mode: str) -> pd.DataFrame:
    ...
    df = df.copy()
    if mode == "Mensile":
        # Normalize to month start for grouping, keep label
        df["__Period"] = pd.to_datetime(df[date_col]).dt.to_period("M").dt.to_timestamp()
        df["__PeriodLabel"] = df["__Period"].dt.strftime("%Y-%m")
    else:
        # Trimestrale
        pr = pd.to_datetime(df[date_col]).dt.to_period("Q")
        df["__Period"] = pr.dt.start_time
        df["__PeriodLabel"] = pr.astype(str)
    return df

def aggregate_amounts(df: pd.DataFrame, by_country_col: str|None = None):
    if df is None or df.empty or "__Period" not in df.columns:
        return pd.DataFrame()
    amount_col = _find_col(df.columns, AMOUNT_IN_COLS)
    msg_col    = _find_col(df.columns, MSG_IN_COLS)
    use_cols = {"__Period":"Period","__PeriodLabel":"PeriodLabel"}
    if amount_col: use_cols[amount_col] = "Amount_EUR"
    if msg_col:    use_cols[msg_col]    = "Messages"
    tmp = df.rename(columns=use_cols)
    group_keys = ["Period","PeriodLabel"]
    if by_country_col:
        group_keys.append(by_country_col)
    agg = tmp.groupby(group_keys, dropna=False).agg(
        Amount_EUR=("Amount_EUR","sum") if "Amount_EUR" in tmp.columns else ("__Period","size"),
        Messages=("Messages","sum") if "Messages" in tmp.columns else ("__Period","size"),
        Rows=("Period","size"),
    ).reset_index()
    return agg

def top_n_by_country(df_agg: pd.DataFrame, by_country_col: str, n:int=10):
    if df_agg.empty or by_country_col not in df_agg.columns:
        return pd.DataFrame()
    tot = df_agg.groupby(by_country_col, as_index=False)[["Amount_EUR"]].sum().sort_values("Amount_EUR", ascending=False)
    return tot.head(n)

def line_chart_amount(df_agg: pd.DataFrame, title: str):
    if df_agg.empty: return None
    base = alt.Chart(df_agg).mark_line(point=True).encode(
        x=alt.X("Period:T", title="Periodo"),
        y=alt.Y("Amount_EUR:Q", title="Importo EUR"),
        tooltip=["PeriodLabel:N","Amount_EUR:Q"]
    ).properties(title=title, height=300)
    return base

def bar_chart_top(df_top: pd.DataFrame, cat_col: str, title: str):
    if df_top.empty: return None
    ch = alt.Chart(df_top).mark_bar().encode(
        x=alt.X("Amount_EUR:Q", title="Importo EUR"),
...
src_mode = st.radio("Sorgente dati", ["Upload file", "URL remoti"], horizontal=True)
...
if src_mode == "Upload file":
    ...
    with col1:
        inbound_url = st.text_input("URL INBOUND (xlsx/xls)", placeholder="https://...")
        if st.button("Leggi INBOUND", key="btn_inb_url") and inbound_url:
            inbound_df = read_excel_from_url(inbound_url)
            if inbound_df is not None:
                st.success(f"INBOUND da URL. Righe: {len(inbound_df)}")
    with col2:
        outbound_url = st.text_input("URL OUTBOUND (xlsx/xls)", placeholder="https://...")
        if st.button("Leggi OUTBOUND", key="btn_outb_url") and outbound_url:
            outbound_df = read_excel_from_url(outbound_url)
            if outbound_df is not None:
                st.success(f"OUTBOUND da URL. Righe: {len(outbound_df)}")
...
inbound_df = None
outbound_df = None
# ⬇️ Applica filtri dinamici post-import (sidebar)
inbound_df = apply_dynamic_filters(inbound_df, "Inbound") if inbound_df is not None else None
outbound_df = apply_dynamic_filters(outbound_df, "Outbound") if outbound_df is not None else None


# ---------------------- Periodicità & Grafici ----------------------
st.subheader("Periodicità & Grafici")
period_mode = st.radio("Periodicità", ["Mensile","Trimestrale"], horizontal=True, key="period_mode")
group_country_col = "Creditor FI Country Name"
st.caption("Raggruppo per: **Creditor FI Country Name** (impostato)")
topn = st.slider("Top N Paesi", 3, 20, 10, 1)

# Prepara periodi
if inbound_df is not None:
    inbound_df = ensure_datetime(inbound_df)
    inbound_df = add_period_column(inbound_df, period_mode)
if outbound_df is not None:
    outbound_df = ensure_datetime(outbound_df)
    outbound_df = add_period_column(outbound_df, period_mode)

# Aggrega per periodo
agg_in  = aggregate_amounts(inbound_df,  by_country_col=group_country_col)  if inbound_df is not None else pd.DataFrame()
agg_out = aggregate_amounts(outbound_df, by_country_col=group_country_col)  if outbound_df is not None else pd.DataFrame()

# Line charts importi per periodo
cols = st.columns(2)
with cols[0]:
    if not agg_in.empty:
        ch = line_chart_amount(agg_in.groupby(["Period","PeriodLabel"], as_index=False)[["Amount_EUR"]].sum(), "INBOUND – Importi per periodo")
        if ch: st.altair_chart(ch, use_container_width=True)
with cols[1]:
    if not agg_out.empty:
        ch = line_chart_amount(agg_out.groupby(["Period","PeriodLabel"], as_index=False)[["Amount_EUR"]].sum(), "OUTBOUND – Importi per periodo")
        if ch: st.altair_chart(ch, use_container_width=True)

# Combined chart (Inbound vs Outbound)
if not agg_in.empty or not agg_out.empty:
    comb = pd.DataFrame()
    if not agg_in.empty:
        a = agg_in.groupby(["Period","PeriodLabel"], as_index=False)[["Amount_EUR"]].sum()
        a["Direction"] = "Inbound"
        comb = pd.concat([comb, a], ignore_index=True)
    if not agg_out.empty:
        b = agg_out.groupby(["Period","PeriodLabel"], as_index=False)[["Amount_EUR"]].sum()
        b["Direction"] = "Outbound"
        comb = pd.concat([comb, b], ignore_index=True)
    if not comb.empty:
        chc = alt.Chart(comb).mark_line(point=True).encode(
            x=alt.X("Period:T", title="Periodo"),
            y=alt.Y("Amount_EUR:Q", title="Importo EUR"),
            color="Direction:N",
            tooltip=["PeriodLabel:N","Direction:N","Amount_EUR:Q"]
        ).properties(title="INBOUND vs OUTBOUND – Importi per periodo", height=320)
        st.altair_chart(chc, use_container_width=True)

# Top N Paesi (sul periodo filtrato totale)
cols2 = st.columns(2)
with cols2[0]:
    top_in = top_n_by_country(agg_in, group_country_col, n=topn)
    if not top_in.empty:
        st.markdown(f"**Top {topn} Paesi – INBOUND**")
        st.dataframe(top_in, use_container_width=True)
        chb = bar_chart_top(top_in, group_country_col, f"Top {topn} Paesi – INBOUND (Importo EUR)")
        if chb: st.altair_chart(chb, use_container_width=True)
with cols2[1]:
    top_out = top_n_by_country(agg_out, group_country_col, n=topn)
    if not top_out.empty:
        st.markdown(f"**Top {topn} Paesi – OUTBOUND**")
        st.dataframe(top_out, use_container_width=True)
        chb = bar_chart_top(top_out, group_country_col, f"Top {topn} Paesi – OUTBOUND (Importo EUR)")
        if chb: st.altair_chart(chb, use_container_width=True)
...
with colhr1:
    policy_upload = st.file_uploader(
        "Carica lista policy (CSV/XLSX) con colonne: Country, Risk/HighRisk",
        type=["csv","xlsx","xls"], key="policy"
    )
    policy_map_uploaded = parse_policy_upload(policy_upload)
    default_sel = []
    policy_sel = st.multiselect(
        "Seleziona/override manuale i Paesi da considerare High Risk",
